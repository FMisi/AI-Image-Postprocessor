...
class ImageProcessorApp:
    def __init__(self, root):
        ...
        window_width, window_height = 600, 880
        ...
        self.alphap_entry.insert(0, "0.0515")
        ...
        self.intensity_entry.insert(0, "0.009")
        ...
        self.softness_entry.insert(0, "2.0")
    ...
    def add_perlin_noise(self, img: Image.Image, scale=50, alphap=0.05, mask=None):
        w, h = img.size
        ...
        for y in range(h):
            for x in range(w):
                noise_map_bg[y][x] = noise.pnoise2(x / scale, y / scale, octaves=2, persistence=0.5, lacunarity=2.0,
        ...
        for y in range(h):
            for x in range(w):
                noise_map_face[y][x] = noise.pnoise2(x / face_scale, y / face_scale, octaves=2, persistence=0.5, lacunarity=2.0,
        ...
        if mask is not None:
            alphap_face = alphap * 0.7
            alphap_bg = alphap

            # Increase blur radius and adjust mask intensity for softer transition
            mask_blurred = cv2.GaussianBlur(mask, (51, 51), 0)
            mask_blurred_norm = mask_blurred.astype(np.float32) / 255.0
            mask_blurred_norm = np.power(mask_blurred_norm, 0.4)  # Softer curve for mask intensity
    ...
    def apply_vignette(self, img: Image.Image, radius=1.5, softness=2.0):
        w, h = img.size
        x = np.linspace(-1, 1, w)
        y = np.linspace(-1, 1, h)
        X, Y = np.meshgrid(x, y)
        ...
        arr = np.array(img).astype(np.float32) / 255.0
        arr = arr * vignette_mask + (1 - vignette_mask)
        arr = np.clip(arr, 0, 1)
        img_out = Image.fromarray((arr * 255).astype(np.uint8))
        return img_out
    ...
    def apply_motion_blur(self, img: Image.Image, degree=5, angle=0):
        ...
        img_out = Image.fromarray(cv2.cvtColor(blurred, cv2.COLOR_BGR2RGB))
        return img_out
    ...
    def detect_face_mask(self, img: Image.Image):
        ...
        # Apply a larger Gaussian blur for a softer edge
    ...
    def process_image(self, img_path, output_folder, alphap, intensity, radius, softness,
        ...
        try:
            ...
            motion_blur_img = self.apply_motion_blur(img_film, degree=7, angle=random.uniform(-15, 15))

            # Softer blending for motion blur
            motion_blur_arr = np.array(motion_blur_img)
            face_mask_3c = np.repeat(face_mask[:, :, np.newaxis] / 255.0, 3, axis=2)
            face_mask_3c = np.power(face_mask_3c, 0.4)  # Softer mask transition
            alpha_mb = 0.2
            ...
            img_vignette = self.apply_vignette(img_blurred, radius=radius, softness=softness)

            # Softer blending for final face integration
            img_np = np.array(img)
            img_final_np = np.array(img_vignette)
            face_mask_3c = np.repeat(face_mask[:, :, np.newaxis] / 255.0, 3, axis=2)
            face_mask_3c = np.power(face_mask_3c, 0.4)  # Softer mask transition
            alpha_face = 0.8  # Control face blending strength
            ...
            if os.path.abspath(input_folder) == os.path.abspath(output_folder):
                new_filename = self.get_unique_filename(output_folder, name, ext)
            else:
